...
if ($id && $slug) {
    // Fetch selected blog
    $stmt = $pdo->prepare("SELECT * FROM articles WHERE id = ? AND slug = ?");
...
<head>
  <meta charset="utf-8">
<title><?= htmlspecialchars($blog['title']) ?> | Flying411</title>
    ...
    <meta name="keywords" content="<?= htmlspecialchars($blog['keywords']) ?>">
    <meta name="author" content="Flying411">

    <!-- Open Graph Meta -->
    <meta property="og:site_name" content="Flying411">
    <meta property="og:title" content="<?= htmlspecialchars($blog['title']) ?> | Flying411">
    <meta property="og:description" content="<?= htmlspecialchars($blog['meta_description']) ?>">
    <meta property="og:type" content="article">
    <meta property="og:image" content="https://flying411.com/assets/pages/blogs/<?= htmlspecialchars($blog['image']) ?>">
    <meta property="og:url" content="https://flying411.com/blog-details.php?id=<?= $blog['id'] ?>&slug=<?= urlencode($blog['slug']) ?>">
...
<div class="my-5" style="width: 90%;margin: auto;">
    <div class="row">
            ...
            <?php foreach ($latestBlogs as $latest): ?>
                <div class="latest-blog mb-4" style="border-bottom: 1px solid #ddd; padding-bottom: 10px;">
                    <a  href="blog-details.php?id=<?= $latest['id'] ?>&slug=<?= urlencode($latest['slug']) ?>">
                        ...
                        <img  src="admin/uploads/<?= htmlspecialchars($latest['image']) ?>" alt="<?= htmlspecialchars($latest['title']) ?>" class="mb-10 img-fluid" >
                         </div>
                    </a>
                     <a style="color:black;" href="blog-details.php?id=<?= $latest['id'] ?>&slug=<?= urlencode($latest['slug']) ?>">
                    <h5 style="width:70%; font-size:15px;" class="mb-1"><?= htmlspecialchars($latest['title']) ?></h5>
                       </a>
        ...
        <!-- Blog Detail Content -->
        <div class="col-lg-8 col-md-7 blogs-data">
            <a href="blogs.php" class="btn btn-sm btn-outline-secondary mb-3">&larr; Back to Blogs</a>

            <div class="mb-4">
                <img src="admin/uploads/<?= htmlspecialchars($blog['image']) ?>" alt="<?= htmlspecialchars($blog['title']) ?>" class="img-fluid rounded w-100" style="max-height: 400px; object-fit: cover;">
            </div>

            <h1 class="mb-10" style="font-size:30px; font-weight:bold;padding:16px 0px;"><?= htmlspecialchars($blog['title']) ?></h1>
            <p style="font-size:15px;line-height:2px;" class="text-muted mb-4"><strong>Published:</strong> <?= date("F j, Y", strtotime($blog['publish_date'])) ?></p>

            <div style="font-size: 16px;line-height: 20px;text-align: justify;" class="blog_content"> 
            ...
            <div class="mt-5 text-end">
                <a href="blogs.php" class="btn btn-danger">Back to Blog List</a>
            </div>
        </div>
    </div>
</div>
